...
const ROLE_ACCESS = {
  BDT_STAFF: { views: ["dashboard", "new-request", "my-requests", "team-requests", "bdt-queue"], defaultView: "dashboard", label: "Market Staff" },
...
const DETAIL_LABELS = { Brand:"Brand", Market:"Market", Account_Type:"Account Type", Call_Request_Type:"Request", Call_Number_Type:"Call Number", Affiliate_Username_2:"Affiliate Username 2", Requested_By_Name:"Requested By", Requested_At:"Requested At", Taken_By_Name:"Handler", Taken_At:"Taken At", BDT_Verified_By_Name:"Market Verifier", BDT_Verified_At:"Verified At", Verification_Remark:"Verification Remark", Completed_By_Name:"Completed By", Completed_At:"Completed At", Unable_Reason:"Unable Reason", Resolution_Remark:"Resolution Remark", Waiting_Seconds:"Waiting Time", Handling_Seconds:"Handling Time", Total_Seconds:"Total Time" };

const $ = (selector, root=document) => root.querySelector(selector);
const $$ = (selector, root=document) => [...root.querySelectorAll(selector)];
const statusBadge = status => `<span class="badge ${String(status).toLowerCase()}">${escapeHtml(status)}</span>`;
const marketBadge = market => `<span class="market-badge">${escapeHtml(market||"BDT")}</span>`;
...
function requestDetailsText(r){return [getField(r,"Player_Username"),getField(r,"Affiliate_Username"),getField(r,"Affiliate_Username_2"),getField(r,"Account_Type"),getField(r,"Call_Request_Type"),getField(r,"Call_Number_Type"),getField(r,"Phone_Number"),getField(r,"Email"),getField(r,"Transaction_ID")].filter(Boolean)}
...
async function loadQueue({silent=false,force=false}={}){if(requestState.loaded.queue){applyQueueFilter();if(!force&&requestState.loading.queue)return;$(".live-indicator").innerHTML="<i></i> Refreshing…"}if(requestState.loading.queue||Object.keys(requestState.mutating).length)return;requestState.loading.queue=true;const initial=!requestState.loaded.queue,version=requestState.queueVersion,sequence=(requestState.sequence.queue||0)+1;requestState.sequence.queue=sequence;if(initial)setTableLoading("#queue-table",10);try{const data=await authenticatedRead("cspQueue",{status:"All",limit:500});if(sequence!==requestState.sequence.queue||version!==requestState.queueVersion)return;requestState.queueMaster=(data.requests||[]).map(normalizeRequest);requestState.loaded.queue=true;applyQueueFilter();saveSessionCache()}catch(e){if(e.code!=="UNAUTHORIZED"&&!silent)showToast(e.message);if(initial)$("#queue-table").innerHTML='<tr class="table-loading"><td colspan="10">Unable to load the CSP queue. <button class="text-button" data-retry-queue>Retry</button></td></tr>';else $(".live-indicator").innerHTML='<span class="refresh-error">Refresh failed —</span> <button class="text-button" data-retry-queue>Retry</button>'}finally{requestState.loading.queue=false}}
function renderBdtQueueRows(){const rows=requestState.bdtQueue,master=requestState.bdtQueueMaster,pending=master.filter(r=>r.status==="Pending").length,processing=master.filter(r=>r.status==="Processing"&&r.raw.Processing_Team==="BDT").length;$("#bdt-queue-count").textContent=pending;$("#bdt-queue-stats").innerHTML=statCards({pending,processing,completedToday:0,unableToday:0});$("#bdt-queue-table").innerHTML=rows.map(r=>{const details=requestDetailsText(r.raw),canFinish=r.status==="Processing"&&r.raw.Processing_Team==="BDT"&&r.takenById===authState.user.userId,handler=r.status==="Processing"?`<strong>Processing by ${escapeHtml(r.takenByName||"Unknown")}</strong>`:"—";return `<tr><td><strong>${ageFrom(r.requestedAt)}</strong></td><td><button class="ticket-button" data-ticket="${escapeHtml(r.requestId)}">${escapeHtml(r.requestId)}</button></td><td>${marketBadge(r.raw.Market)}</td><td><strong>${escapeHtml(r.brand)}</strong></td><td>${escapeHtml(r.requestType)}</td><td><strong>${escapeHtml(details[0]||"—")}</strong><span class="sub-detail">${escapeHtml(details.slice(1).join(" / ")||"—")}</span></td><td>${escapeHtml(r.requestedByName||"—")}</td><td>${statusBadge(r.status)}</td><td>${handler}</td><td><div class="action-group"><button class="action-button" data-ticket="${escapeHtml(r.requestId)}">View</button>${r.status==="Pending"?`<button class="action-button primary" data-take="${escapeHtml(r.requestId)}">Take Request</button>`:canFinish?`<button class="action-button success" data-verify="${escapeHtml(r.requestId)}">Verified</button><button class="action-button danger" data-unable="${escapeHtml(r.requestId)}">Unable</button>`:""}</div></td></tr>`}).join("");$("#bdt-queue-empty").hidden=rows.length>0;$("#bdt-live-indicator").innerHTML='<i></i> Last updated '+new Date().toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"})}
...
function applyBdtQueueFilter(){const status=requestState.bdtQueueStatus,search=($("#bdt-queue-search")?.value||"").trim().toLowerCase();requestState.bdtQueue=requestState.bdtQueueMaster.filter(row=>(status==="All"||(status==="Active"?(row.status==="Pending"||(row.status==="Processing"&&row.raw.Processing_Team==="BDT")):row.status===status&&(status!=="Processing"||row.raw.Processing_Team==="BDT")))&&(!search||requestDetailsText(row.raw).concat([row.requestId,row.brand,row.requestType,row.requestedByName,row.takenByName]).join(" ").toLowerCase().includes(search)));renderBdtQueueRows()}
async function loadBdtQueue({silent=false,force=false}={}){if(requestState.loaded.bdtQueue){applyBdtQueueFilter();if(!force&&requestState.loading.bdtQueue)return;$("#bdt-live-indicator").innerHTML="<i></i> Refreshing…"}if(requestState.loading.bdtQueue||Object.keys(requestState.mutating).length)return;requestState.loading.bdtQueue=true;const initial=!requestState.loaded.bdtQueue,version=requestState.queueVersion,sequence=(requestState.sequence.bdtQueue||0)+1;requestState.sequence.bdtQueue=sequence;if(initial)setTableLoading("#bdt-queue-table",10);try{const data=await authenticatedRead("bdtQueue",{status:"All",limit:500});if(sequence!==requestState.sequence.bdtQueue||version!==requestState.queueVersion)return;requestState.bdtQueueMaster=(data.requests||[]).map(normalizeRequest);requestState.loaded.bdtQueue=true;applyBdtQueueFilter()}catch(e){if(e.code!=="UNAUTHORIZED"&&!silent)showToast(e.message);if(initial)$("#bdt-queue-table").innerHTML='<tr class="table-loading"><td colspan="10">Unable to load CSP requests. <button class="text-button" data-retry-bdt-queue>Retry</button></td></tr>';else $("#bdt-live-indicator").innerHTML='<span class="refresh-error">Refresh failed —</span> <button class="text-button" data-retry-bdt-queue>Retry</button>'}finally{requestState.loading.bdtQueue=false}}
function renderCspRequests(){const status=requestState.cspRequestsStatus,rows=requestState.cspRequests.filter(r=>status==="All"||getField(r,"Status")===status);$$('[data-csp-requests-status]').forEach(button=>button.classList.toggle("active",button.dataset.cspRequestsStatus===status));$("#csp-requests-table").innerHTML=rows.map(r=>{const id=getField(r,"Request_ID"),state=getField(r,"Status"),details=requestDetailsText(r),canComplete=state==="Processing"&&getField(r,"Processing_Team")==="CSP"&&getField(r,"Taken_By_ID")===authState.user.userId,action=state==="Verified"?`<button class="action-button primary" data-take="${escapeHtml(id)}">Take Request</button>`:canComplete?`<button class="action-button success" data-complete="${escapeHtml(id)}">Complete</button>`:"";return `<tr><td><button class="ticket-button" data-ticket="${escapeHtml(id)}">${escapeHtml(id)}</button></td><td>${marketBadge(getField(r,"Market"))}</td><td>${escapeHtml(getField(r,"Request_Type"))}</td><td><strong>${escapeHtml(details[0]||getField(r,"Account_Type")||"—")}</strong><span class="sub-detail">${escapeHtml(details.slice(1).join(" / ")||getField(r,"Call_Request_Type")||"—")}</span></td><td>${statusBadge(state)}</td><td>${escapeHtml(formatDate(getField(r,"Requested_At")))}</td><td>${escapeHtml(getField(r,"BDT_Verified_By_Name")||getField(r,"Taken_By_Name")||"—")}</td><td>${escapeHtml(getField(r,"Verification_Remark")||getField(r,"Resolution_Remark")||"—")}</td><td><div class="action-group"><button class="action-button" data-ticket="${escapeHtml(id)}">View</button>${action}</div></td></tr>`}).join("");$("#csp-requests-summary").innerHTML=`Showing <strong>${rows.length}</strong> of ${requestState.cspRequests.length} requests${requestState.loading.cspRequests?' <span class="refreshing-label">Refreshing…</span>':""}`;$("#csp-requests-empty").hidden=rows.length>0}
async function loadCspRequests({silent=false,force=false}={}){if(requestState.loading.cspRequests)return;if(requestState.loaded.cspRequests){renderCspRequests();if(!force)return}requestState.loading.cspRequests=true;const initial=!requestState.loaded.cspRequests,version=requestState.queueVersion,sequence=(requestState.sequence.cspRequests||0)+1;requestState.sequence.cspRequests=sequence;if(initial)setTableLoading("#csp-requests-table",9);else renderCspRequests();try{const data=await authenticatedRead("cspRequests",{limit:500});if(sequence!==requestState.sequence.cspRequests||version!==requestState.queueVersion)return;requestState.cspRequests=data.requests||[];requestState.loaded.cspRequests=true;saveSessionCache()}catch(e){if(e.code!=="UNAUTHORIZED"&&!silent)showToast(e.message);if(initial)$("#csp-requests-table").innerHTML='<tr class="table-loading"><td colspan="9">Unable to load CSP requests.</td></tr>'}finally{requestState.loading.cspRequests=false;if(requestState.loaded.cspRequests)renderCspRequests()}}
...
function cspCaseSubmissionMatch(row,payload,started){return new Date(row.Requested_At).getTime()>=started-5000&&row.Brand===payload.brand&&row.Market===payload.market&&row.Request_Type===payload.requestType&&(!payload.playerUsername||row.Player_Username===payload.playerUsername)&&(!payload.affiliateUsername||row.Affiliate_Username===payload.affiliateUsername)&&(!payload.affiliateUsername2||row.Affiliate_Username_2===payload.affiliateUsername2)&&row.Requested_By_ID===authState.user.userId}
...
function showCreatedCspCase(data){showToast(`${data.ticket} created — sent to ${data.request?.Market||"market staff"} as Pending`);if(data.request){requestState.queueVersion+=1;requestState.cspRequests.unshift(data.request);requestState.loaded.cspRequests=true;requestState.detailCache[data.ticket]={request:data.request,history:[]};saveSessionCache();openDetails(data.ticket);setTimeout(()=>loadDashboard({silent:true}),0)}}
async function submitCspCase(event){event.preventDefault();const form=event.currentTarget;if(!form.checkValidity()){form.reportValidity();return}const type=$("#csp-case-type").value,payload={brand:$("#csp-case-brand").value,market:$("#csp-case-market").value,requestType:type,playerUsername:$("#csp-case-player")?.value.trim()||"",affiliateUsername:$("#csp-case-affiliate")?.value.trim()||"",affiliateUsername2:$("#csp-case-affiliate-2")?.value.trim()||"",currentCurrency:$("#csp-case-current-currency")?.value.trim()||"",correctCurrency:$("#csp-case-correct-currency")?.value.trim()||"",account1Email:$("#csp-case-account1-email")?.value.trim()||"",account1Phone:$("#csp-case-account1-phone")?.value.trim()||"",account2Email:$("#csp-case-account2-email")?.value.trim()||"",account2Phone:$("#csp-case-account2-phone")?.value.trim()||"",currentFullName:$("#csp-case-current-name")?.value.trim()||"",newFullName:$("#csp-case-new-name")?.value.trim()||"",currentDob:$("#csp-case-current-dob")?.value||"",newDob:$("#csp-case-new-dob")?.value||"",accountType:$("#csp-case-account-type")?.value||"",callRequestType:$("#csp-case-call-request")?.value||"",callNumberType:$("#csp-case-call-number")?.value||"",notes:$("#csp-case-notes")?.value.trim()||""},started=Date.now();setFormBusy(form,true,"Creating…");try{payload.attachment=await cspAttachmentPayload();const data=await authenticatedPost("createCspCase",payload);form.reset();renderMarketCspCaseFields("");showCreatedCspCase(data)}catch(e){if(["TIMEOUT","UPSTREAM_TIMEOUT"].includes(e.code)){showToast("Confirming whether your CSP request was received…");const committed=await reconcileCspCase(payload,started);if(committed){form.reset();renderMarketCspCaseFields("");showCreatedCspCase({ticket:committed.Request_ID,request:committed});return}showToast("Submission could not be confirmed. Check CSP Requests before trying again.")}else showToast(e.message)}finally{setFormBusy(form,false)}}
...
function reportFilters(){return {fromDate:$("#report-from").value,toDate:$("#report-to").value,market:$("#report-market").value,brand:$("#report-brand").value,requestType:$("#report-type").value,status:$("#report-status").value,requestedBy:$("#report-requester").value.trim(),handledBy:$("#report-handler").value.trim(),limit:500}}
function reportFilterLabels(){const f=reportFilters(),labels=[];if(f.fromDate||f.toDate)labels.push(`Date: ${f.fromDate||"Any"} → ${f.toDate||"Any"}`);if(f.market)labels.push(`Market: ${f.market}`);if(f.brand)labels.push(`Brand: ${f.brand}`);if(f.requestType)labels.push(`Type: ${f.requestType}`);if(f.status)labels.push(`Status: ${f.status}`);if(f.requestedBy)labels.push(`Requested By: ${f.requestedBy}`);if(f.handledBy)labels.push(`Handled By: ${f.handledBy}`);return labels}
...
function resetReportFilters(){["report-from","report-to","report-market","report-brand","report-type","report-status","report-requester","report-handler"].forEach(id=>{$(`#${id}`).value=""});renderReportFilters();loadReports()}
function updateReportEmpty(){if(requestState.reports.length)return;$("#report-table").innerHTML='<tr><td colspan="9"><div class="report-empty"><strong>No requests match the current filters.</strong><span>Review the active filters above or clear them to see all available requests.</span><button class="secondary-button" data-clear-report-filters type="button">Clear Filters</button></div></td></tr>'}
...
function detailsActionBar(raw){const r=normalizeRequest(raw),csp=["CSP_STAFF","CSP_ADMIN","SUPER_ADMIN"].includes(authState.user.role);if(raw.CSP_Case){if(raw.Processing_Team==="BDT"){if(authState.user.role!=="BDT_STAFF")return "";if(r.status==="Pending")return `<div class="details-action-bar"><button class="primary-button" data-take="${escapeHtml(r.requestId)}">Take Request</button></div>`;if(r.status==="Processing"&&r.takenById===authState.user.userId)return `<div class="details-action-bar"><span>Processing by ${escapeHtml(r.takenByName||authState.user.name)}</span><div><button class="action-button success" data-verify="${escapeHtml(r.requestId)}">Verified</button><button class="action-button danger" data-unable="${escapeHtml(r.requestId)}">Unable</button></div></div>`;if(r.status==="Processing")return `<div class="details-action-bar"><span>Processing by ${escapeHtml(r.takenByName||"another BDT staff member")}</span></div>`;return ""}if(!csp)return "";if(r.status==="Verified")return `<div class="details-action-bar"><button class="primary-button" data-take="${escapeHtml(r.requestId)}">Take Request</button></div>`;if(r.status==="Processing"&&r.takenById===authState.user.userId)return `<div class="details-action-bar"><span>Processing by ${escapeHtml(r.takenByName||authState.user.name)}</span><button class="action-button success" data-complete="${escapeHtml(r.requestId)}">Complete</button></div>`;if(r.status==="Processing")return `<div class="details-action-bar"><span>Processing by ${escapeHtml(r.takenByName||"another CSP staff member")}</span></div>`;return ""}if(!csp)return "";const canFinish=r.status==="Processing"&&(authState.user.role!=="CSP_STAFF"||r.takenById===authState.user.userId);if(r.status==="Pending")return `<div class="details-action-bar"><button class="primary-button" data-take="${escapeHtml(r.requestId)}">Take Request</button></div>`;if(canFinish)return `<div class="details-action-bar"><span>Processing by ${escapeHtml(r.takenByName||authState.user.name)}</span><div><button class="action-button success" data-complete="${escapeHtml(r.requestId)}">Complete</button><button class="action-button danger" data-unable="${escapeHtml(r.requestId)}">Unable</button></div></div>`;if(r.status==="Processing")return `<div class="details-action-bar"><span>Processing by ${escapeHtml(r.takenByName||"another CSP staff member")}</span></div>`;return ""}
...
function detailSection(title,keys,r,timing=false){const fields=keys.filter(key=>r[key]!==""&&r[key]!=null);if(!fields.length)return "";return `<section class="detail-section"><h3>${escapeHtml(title)}</h3><div class="detail-grid">${fields.map(key=>{const label=key==="Affiliate_Username"&&r.Request_Type==="MAC Signup"?"Affiliate Username 1":FIELD_META[key]?.[1]||DETAIL_LABELS[key]||key.replaceAll("_"," "),value=timing?formatDuration(r[key]):key.endsWith("_At")?formatDate(r[key]):r[key],copy=!!FIELD_META[key]&&!timing,wide=key==="Notes"||key==="Unable_Reason"||key==="Resolution_Remark"||key==="Verification_Remark";return `<div class="detail-item${wide?" detail-item-wide":""}"><label>${escapeHtml(label)}</label><div class="detail-value"><span>${escapeHtml(value)}</span>${copy?`<button class="copy-button" data-copy="${escapeHtml(value)}">Copy</button>`:""}</div></div>`}).join("")}</div></section>`}
function timelineTone(history){const value=String(history.New_Status||history.Action||"").toLowerCase();return value.includes("complete")?"completed":value.includes("unable")?"unable":value.includes("cancel")?"cancelled":value.includes("verified")?"verified":value.includes("taken")||value.includes("process")?"processing":"submitted"}
...
async function openAttachment(ticket){$("#attachment-file-name").textContent="Loading attachment…";$("#attachment-image").removeAttribute("src");$("#view-full-attachment").hidden=true;openModal("attachment-modal");try{const data=await authenticatedPost("requestAttachment",{requestId:ticket});$("#attachment-file-name").textContent=data.fileName||"Attachment";$("#attachment-image").src=`data:${data.mimeType};base64,${data.base64}`;$("#view-full-attachment").hidden=false}catch(e){closeModals();showToast(e.message)}}
function openFullAttachment(){const source=$("#attachment-image").getAttribute("src");if(!source)return;$("#attachment-full-image").src=source;openModal("attachment-full-modal")}
function closeFullAttachment(){$("#attachment-full-modal").hidden=true;$("#attachment-full-image").removeAttribute("src");document.body.style.overflow=$("#attachment-modal").hidden?"":"hidden"}
...
function closeModals(){ $$(".modal-backdrop").forEach(m=>m.hidden=true); $("#attachment-image")?.removeAttribute("src"); $("#attachment-full-image")?.removeAttribute("src"); $("#view-full-attachment").hidden=true; clearSensitiveFields(); document.body.style.overflow=""; }
...
function enforceTeamForRole(roleSelect, teamSelect) {
  const expected = { CSP_STAFF: "CSP", CSP_ADMIN: "CSP", SUPER_ADMIN: "ADMIN" };
...
async function mutateRequest(action, requestId, payload, successMessage, button, options={}) {
  const busyLabels={takeRequest:"Taking…",verifyRequest:"Verifying…",completeRequest:"Completing…",unableRequest:"Updating…",cancelRequest:"Cancelling…"};
...
function confirmRequestAction(title,copy,label,callback,showResolution=false,remarkLabel="Resolution Remark"){$("#request-confirm-title").textContent=title;$("#request-confirm-copy").textContent=copy;$("#complete-resolution-field").hidden=!showResolution;$('label[for="complete-resolution-remark"]').textContent=remarkLabel;const remark=$("#complete-resolution-remark");remark.value="";remark.placeholder=remarkLabel==="Verification Remark"?"Add verification outcome or guidance for CSP...":"Add completion remarks or guidance for CSP...";const button=$("#request-confirm-button");button.textContent=label;button.onclick=()=>callback(button);openModal("request-confirm-modal")}
...
function initialize() {
  ...
  document.addEventListener("click",e=>{const metric=e.target.closest("[data-metric-status]")?.dataset.metricStatus;if(metric){if(e.target.closest("#bdt-queue-view")){requestState.bdtQueueStatus=metric;navigate("bdt-queue")}else if(["CSP_STAFF","CSP_ADMIN","SUPER_ADMIN"].includes(authState.user.role)){requestState.queueStatus=metric;navigate("csp-queue")}else{$("#my-status").value=metric;navigate("my-requests")}}const ticket=e.target.closest("[data-ticket]")?.dataset.ticket;if(ticket)openDetails(ticket);const attachment=e.target.closest("[data-view-attachment]")?.dataset.viewAttachment;if(attachment)openAttachment(attachment);if(e.target.closest("[data-view-full-attachment]"))openFullAttachment();if(e.target.closest("[data-close-full-attachment]")||e.target.id==="attachment-full-modal")closeFullAttachment();const inboxThread=e.target.closest("[data-inbox-thread]")?.dataset.inboxThread;if(inboxThread)openInboxThread(inboxThread);const retryDetails=e.target.closest("[data-retry-details]")?.dataset.retryDetails;if(retryDetails)openDetails(retryDetails);const handlerRole=["BDT_STAFF","CSP_STAFF","CSP_ADMIN","SUPER_ADMIN"].includes(authState.user.role);const take=e.target.closest("[data-take]")?.dataset.take;if(take&&handlerRole)mutateRequest("takeRequest",take,{},`Request ${take} is now processing.`,e.target);const verify=e.target.closest("[data-verify]")?.dataset.verify;if(verify&&authState.user.role==="BDT_STAFF"){const reopen=!!e.target.closest("#details-modal");confirmRequestAction("Verify Request",`Mark ${verify} as verified and return it to CSP?`,"Mark Verified",button=>mutateRequest("verifyRequest",verify,{remark:$("#complete-resolution-remark").value.trim()},`Request ${verify} verified and returned to CSP.`,button,{reopenDetails:reopen}),true,"Verification Remark")}const complete=e.target.closest("[data-complete]")?.dataset.complete;if(complete&&handlerRole){const reopen=!!e.target.closest("#details-modal"),cspCase=!!cachedRequest(complete)?.CSP_Case;confirmRequestAction("Complete Request",`Mark ${complete} as completed?`,"Complete Request",button=>mutateRequest("completeRequest",complete,cspCase?{remark:$("#complete-resolution-remark").value.trim()}:{},`Request ${complete} completed.`,button,{reopenDetails:reopen}),cspCase)}const unable=e.target.closest("[data-unable]")?.dataset.unable;if(unable&&handlerRole){requestState.reopenAfterMutation=!!e.target.closest("#details-modal");$("#unable-ticket").value=unable;$("#unable-resolution-field").hidden=cachedRequest(unable)?.Processing_Team!=="BDT";$("#unable-resolution-remark").value="";openModal("unable-modal")}const cancel=e.target.closest("[data-cancel]")?.dataset.cancel;if(cancel&&["BDT_STAFF","SUPER_ADMIN"].includes(authState.user.role))confirmRequestAction("Cancel Request",`Cancel pending request ${cancel}?`,"Cancel Request",button=>mutateRequest("cancelRequest",cancel,{},`Request ${cancel} cancelled.`,button));if(e.target.closest("[data-retry-queue]"))loadQueue();if(e.target.closest("[data-retry-bdt-queue]"))loadBdtQueue();if(e.target.closest("[data-close-modal]")||(e.target.classList.contains("modal-backdrop")&&e.target.id!=="attachment-full-modal"))closeModals();const copy=e.target.closest("[data-copy]")?.dataset.copy;if(copy)copyValue(copy)});
  ...
  document.addEventListener("keydown",e=>{if(e.key==="Escape"){if(!$("#attachment-full-modal").hidden)closeFullAttachment();else closeModals()}});
