...
const requestState = { dashboard: null, my: [], team: [], queue: [], reports: [], queueStatus: "Active", loading: {}, loaded: {}, sequence: {}, mutating: {}, duplicatePayload: null, submittedTicket: null, activeDetailId: null, refreshTimer: null, lastAutoRefresh: 0, filterTimers: {} };
const API_URL = "/api";
...
async function loadQueue({silent=false}={}){if(requestState.loading.queue||requestState.mutating.queue)return;requestState.loading.queue=true;const initial=!requestState.loaded.queue,sequence=(requestState.sequence.queue||0)+1;requestState.sequence.queue=sequence;if(initial)setTableLoading("#queue-table",9);try{const data=await authenticatedPost("cspQueue",{status:requestState.queueStatus,search:$("#queue-search")?.value||"",limit:200});if(sequence!==requestState.sequence.queue)return;requestState.queue=(data.requests||[]).map(normalizeRequest);requestState.loaded.queue=true;renderQueueRows();}catch(e){if(e.code!=="UNAUTHORIZED"&&!silent)showToast(e.message);if(initial){$("#queue-table").innerHTML='<tr class="table-loading"><td colspan="9">Unable to load the CSP queue. <button class="text-button" data-retry-queue>Retry</button></td></tr>';}}finally{requestState.loading.queue=false}}
async function loadCatalogs(){if(catalogState.loading||catalogState.loaded)return;catalogState.loading=true;try{const [brands,types]=await Promise.all([apiGet("brands"),apiGet("requestTypes")]);catalogState.brands=brands||[];catalogState.requestTypes=types||[];catalogState.loaded=true;const brandCodes=catalogState.brands.map(x=>x.code);["brand","my-brand","team-brand","report-brand"].forEach(id=>replaceOptions($(`#${id}`),brandCodes,id==="brand"?"Select brand":"All brands"));const typeNames=catalogState.requestTypes.map(x=>x.requestType);["request-type","my-type","team-type","report-type"].forEach(id=>{const el=$(`#${id}`);if(el)replaceOptions(el,typeNames,id==="request-type"?"Select request type":"All request types")});}catch(e){showToast(e.message)}finally{catalogState.loading=false}}
...
function initialize() {
  ...
  $("#queue-status-tabs").addEventListener("click",e=>{const status=e.target.closest("[data-queue-status]")?.dataset.queueStatus;if(!status)return;requestState.queueStatus=status;$$('[data-queue-status]').forEach(b=>b.classList.toggle("active",b.dataset.queueStatus===status));loadQueue({silent:requestState.loaded.queue})});
  ...
  $("#run-reports").addEventListener("click",loadReports);$("#export-reports").addEventListener("click",exportReports);
