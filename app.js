...
function populateCatalogs(scope="all"){if(scope!=="types"){const brandCodes=catalogState.brands.map(x=>x.code);["brand","my-brand","team-brand","report-brand"].forEach(id=>replaceOptions($(`#${id}`),brandCodes,id==="brand"?"Select brand":"All brands"))}if(scope!=="brands"){const typeNames=catalogState.requestTypes.map(x=>x.requestType);["request-type","my-type","team-type","report-type"].forEach(id=>replaceOptions($(`#${id}`),typeNames,id==="request-type"?"Select request type":"All request types"))}}
function setCatalogLoading(id,loading,label){const select=$(`#${id}`);if(!select)return;select.disabled=loading;if(loading)select.innerHTML=`<option value="">Loading ${label}…</option>`}
async function loadCatalogs(){if(catalogState.loaded){populateCatalogs();return}if(catalogState.loading)return;catalogState.loading=true;if(!catalogState.brands.length)setCatalogLoading("brand",true,"brands");if(!catalogState.requestTypes.length)setCatalogLoading("request-type",true,"request types");try{await Promise.all([authenticatedRead("brands").then(brands=>{catalogState.brands=brands||[];populateCatalogs("brands");setCatalogLoading("brand",false)}),authenticatedRead("requestTypes").then(types=>{catalogState.requestTypes=types||[];populateCatalogs("types");setCatalogLoading("request-type",false)})]);catalogState.loaded=true;saveSessionCache()}catch(e){showToast(e.message)}finally{catalogState.loading=false;setCatalogLoading("brand",false);setCatalogLoading("request-type",false);populateCatalogs()}}
function replaceOptions(select,values,first){if(!select)return;const current=select.value,existing=[...select.options].slice(1).map(option=>option.value);if(select.options[0]?.textContent===first&&existing.length===values.length&&existing.every((value,index)=>value===values[index]))return;select.innerHTML=`<option value="">${first}</option>`;fillSelect(select,values);if(values.includes(current))select.value=current}
function renderDynamicFields(type){const container=$("#dynamic-fields");if(container.dataset.requestType===type)return;const values=Object.fromEntries($$("input, textarea, select",container).map(field=>[field.name,field.value])),definition=catalogState.requestTypes.find(x=>x.requestType===type);container.dataset.requestType=type;if(!definition){container.innerHTML='<div class="form-placeholder"><span>↖</span><p>Select a request type to see the required details.</p></div>';return}const required=definition.requiredFields||[],fields=[...required,...(definition.optionalFields||[]).filter(x=>!required.includes(x))].filter(x=>FIELD_META[x]);container.innerHTML=`<div class="form-section-title"><span>2</span><div><h3>Request details</h3><p>Provide accurate information for ${escapeHtml(type.toLowerCase())}.</p></div></div><div class="form-grid two">${fields.map(name=>{const [key,label,inputType]=FIELD_META[name],req=required.includes(name);return `<div class="field ${inputType==="textarea"?"full-width":""}"><label for="field-${key}">${label}${req?' <em>*</em>':''}</label>${inputType==="textarea"?`<textarea id="field-${key}" name="${key}" rows="3" ${req?"required":""}></textarea>`:`<input id="field-${key}" name="${key}" type="${inputType}" ${req?"required":""}>`}<small class="error-text">This field is required.</small></div>`}).join("")}</div>`;$$('input, textarea, select',container).forEach(field=>{if(values[field.name]!=null)field.value=values[field.name]})}
