...
function startAutoRefresh(){clearInterval(requestState.refreshTimer);requestState.lastAutoRefresh=Date.now();requestState.refreshTimer=setInterval(()=>{if(!authState.user||document.hidden||Object.keys(requestState.mutating).length)return;const view=location.hash.slice(1),threshold=view==="csp-queue"?20000:30000;if(Date.now()-requestState.lastAutoRefresh<threshold)return;requestState.lastAutoRefresh=Date.now();if(view==="csp-queue")loadQueue({silent:true,force:true});else if(view==="dashboard")loadDashboard({silent:true});else if(view==="my-requests")loadRequestList("my",{silent:true});else if(view==="team-requests")loadRequestList("team",{silent:true});else if(view==="inbox")loadInbox({silent:true})},5000)}
...
function initialize() {
  ...
  document.addEventListener("click",e=>{const metric=e.target.closest("[data-metric-status]")?.dataset.metricStatus;if(metric){if(["CSP_STAFF","CSP_ADMIN","SUPER_ADMIN"].includes(authState.user.role)){requestState.queueStatus=metric;navigate("csp-queue")}else{$("#my-status").value=metric;navigate("my-requests")}}const ticket=e.target.closest("[data-ticket]")?.dataset.ticket;if(ticket)openDetails(ticket);const inboxThread=e.target.closest("[data-inbox-thread]")?.dataset.inboxThread;if(inboxThread)openInboxThread(inboxThread);const retryDetails=e.target.closest("[data-retry-details]")?.dataset.retryDetails;if(retryDetails)openDetails(retryDetails);const take=e.target.closest("[data-take]")?.dataset.take;if(take&&["CSP_STAFF","CSP_ADMIN","SUPER_ADMIN"].includes(authState.user.role))mutateRequest("takeRequest",take,{},`Request ${take} is now processing.`,e.target);const complete=e.target.closest("[data-complete]")?.dataset.complete;if(complete&&["CSP_STAFF","CSP_ADMIN","SUPER_ADMIN"].includes(authState.user.role)){const reopen=!!e.target.closest("#details-modal");confirmRequestAction("Complete Request",`Mark ${complete} as completed?`,"Complete Request",button=>mutateRequest("completeRequest",complete,{},`Request ${complete} completed.`,button,{reopenDetails:reopen}))}const unable=e.target.closest("[data-unable]")?.dataset.unable;if(unable&&["CSP_STAFF","CSP_ADMIN","SUPER_ADMIN"].includes(authState.user.role)){requestState.reopenAfterMutation=!!e.target.closest("#details-modal");$("#unable-ticket").value=unable;openModal("unable-modal")}const cancel=e.target.closest("[data-cancel]")?.dataset.cancel;if(cancel&&["BDT_STAFF","SUPER_ADMIN"].includes(authState.user.role))confirmRequestAction("Cancel Request",`Cancel pending request ${cancel}?`,"Cancel Request",button=>mutateRequest("cancelRequest",cancel,{},`Request ${cancel} cancelled.`,button));if(e.target.closest("[data-retry-queue]"))loadQueue();if(e.target.closest("[data-close-modal]")||e.target.classList.contains("modal-backdrop"))closeModals();const copy=e.target.closest("[data-copy]")?.dataset.copy;if(copy)copyValue(copy)});
