...
function renderRequestList(kind){const team=kind==="team";$( `#${kind}-requests-table`).innerHTML=requestState[kind].map(r=>liveRow(r,team,!team)).join("");$(`#${kind}-result-summary`).textContent=`Showing ${requestState[kind].length} request${requestState[kind].length===1?"":"s"}`;$(`#${kind}-empty`).hidden=requestState[kind].length>0}
async function loadRequestList(kind,{silent=false}={}){if(requestState.loading[kind])return;requestState.loading[kind]=true;const team=kind==="team",initial=!requestState.loaded[kind],sequence=(requestState.sequence[kind]||0)+1,filters=requestFilters(kind);requestState.sequence[kind]=sequence;if(initial)setTableLoading(`#${kind}-requests-table`,team?9:8);try{const data=await authenticatedRead(team?"teamRequests":"myRequests",filters);if(sequence!==requestState.sequence[kind])return;requestState[kind]=data.requests||[];requestState.loaded[kind]=true;renderRequestList(kind);saveSessionCache()}catch(e){if(e.code!=="UNAUTHORIZED"&&!silent)showToast(e.message);if(initial){$( `#${kind}-requests-table`).innerHTML="";$(`#${kind}-result-summary`).textContent=`Unable to load ${team?"team":"your"} requests.`;$(`#${kind}-empty`).hidden=false;}}finally{requestState.loading[kind]=false}}
