...
const requestState = { dashboard: null, my: [], team: [], queue: [], queueMaster: [], bdtQueue: [], bdtQueueMaster: [], reports: [], reportCache: {}, activeReportKey: "", detailCache: {}, queueStatus: "Active", bdtQueueStatus: "Active", queueVersion: 0, loading: {}, loaded: {}, sequence: {}, inFlight: {}, mutating: {}, duplicatePayload: null, submittedTicket: null, activeDetailId: null, refreshTimer: null, lastAutoRefresh: 0, filterTimers: {} };
...
const ROLE_ACCESS = {
  ...
  CSP_STAFF: { views: ["dashboard", "csp-queue", "inbox", "ticket-search", "new-csp-case"], defaultView: "csp-queue", label: "CSP Staff" },
  CSP_ADMIN: { views: ["dashboard", "csp-queue", "inbox", "ticket-search", "new-csp-case", "reports"], defaultView: "csp-queue", label: "CSP Admin" },
  SUPER_ADMIN: { views: ["dashboard", "new-request", "my-requests", "team-requests", "csp-queue", "inbox", "ticket-search", "new-csp-case", "reports", "user-management"], defaultView: "dashboard", label: "Super Admin" }
};
...
function renderCspCaseFields(type){const container=$("#csp-case-fields"),notes='<div class="field full-width"><label for="csp-case-notes">Notes</label><textarea id="csp-case-notes" rows="3" maxlength="2000"></textarea></div>';if(type==="Customer Available for Call")container.innerHTML=`<div class="form-section-title"><span>2</span><div><h3>Case details</h3><p>Identify the available customer.</p></div></div><div class="form-grid two"><div class="field"><label for="csp-case-subject-type">Subject Type <em>*</em></label><select id="csp-case-subject-type" required><option value="">Select type</option><option>Player</option><option>Affiliate</option></select></div><div class="field"><label for="csp-case-username">Username <em>*</em></label><input id="csp-case-username" required maxlength="150"></div>${notes}</div>`;else if(type==="Wrong Currency Signup")container.innerHTML=`<div class="form-section-title"><span>2</span><div><h3>Case details</h3><p>Record the currency correction.</p></div></div><div class="form-grid two"><div class="field"><label for="csp-case-affiliate">Affiliate Username <em>*</em></label><input id="csp-case-affiliate" required maxlength="150"></div><div></div><div class="field"><label for="csp-case-current-currency">Current Currency <em>*</em></label><input id="csp-case-current-currency" required maxlength="30"></div><div class="field"><label for="csp-case-correct-currency">Correct Currency <em>*</em></label><input id="csp-case-correct-currency" required maxlength="30"></div>${notes}</div>`;else if(type==="High Balance Unlock / Verify")container.innerHTML=`<div class="form-section-title"><span>2</span><div><h3>Case details</h3><p>Identify the player account.</p></div></div><div class="form-grid two"><div class="field"><label for="csp-case-player">Player Username <em>*</em></label><input id="csp-case-player" required maxlength="150"></div>${notes}</div>`;else container.innerHTML='<div class="form-placeholder"><span>↖</span><p>Select a case type to see the required details.</p></div>'}
async function submitCspCase(event){event.preventDefault();const form=event.currentTarget;if(!form.checkValidity()){form.reportValidity();return}const type=$("#csp-case-type").value,subjectType=$("#csp-case-subject-type")?.value||"",username=$("#csp-case-username")?.value.trim()||"",payload={brand:$("#csp-case-brand").value,requestType:type,subjectType,playerUsername:subjectType==="Player"?username:($("#csp-case-player")?.value.trim()||""),affiliateUsername:subjectType==="Affiliate"?username:($("#csp-case-affiliate")?.value.trim()||""),currentCurrency:$("#csp-case-current-currency")?.value.trim()||"",correctCurrency:$("#csp-case-correct-currency")?.value.trim()||"",notes:$("#csp-case-notes")?.value.trim()||""};setFormBusy(form,true,"Creating…");try{const data=await authenticatedPost("createCspCase",payload);form.reset();renderCspCaseFields("");showToast(`${data.ticket} created — sent to BDT as Pending`);if(data.request){requestState.queueVersion+=1;requestState.detailCache[data.ticket]={request:data.request,history:[]};saveSessionCache();openDetails(data.ticket);setTimeout(()=>loadDashboard({silent:true}),0)}}catch(e){showToast(e.message)}finally{setFormBusy(form,false)}}
...
function initialize() {
  ...
  $("#unable-form").addEventListener("submit",async e=>{e.preventDefault();if(!e.currentTarget.checkValidity()){e.currentTarget.reportValidity();return}const reason=$("#unable-reason").value==="Other"?$("#other-reason").value.trim():$("#unable-reason").value,updated=await mutateRequest("unableRequest",$("#unable-ticket").value,{reason},`Request ${$("#unable-ticket").value} marked unable.`,$('button[type="submit"]',e.currentTarget),{reopenDetails:requestState.reopenAfterMutation});if(updated){requestState.reopenAfterMutation=false;e.currentTarget.reset();$("#other-reason-field").hidden=true}});
  ...
  document.addEventListener("click",e=>{const metric=e.target.closest("[data-metric-status]")?.dataset.metricStatus;if(metric){if(e.target.closest("#bdt-queue-view")){requestState.bdtQueueStatus=metric;navigate("bdt-queue")}else if(["CSP_STAFF","CSP_ADMIN","SUPER_ADMIN"].includes(authState.user.role)){requestState.queueStatus=metric;navigate("csp-queue")}else{$("#my-status").value=metric;navigate("my-requests")}}const ticket=e.target.closest("[data-ticket]")?.dataset.ticket;if(ticket)openDetails(ticket);const inboxThread=e.target.closest("[data-inbox-thread]")?.dataset.inboxThread;if(inboxThread)openInboxThread(inboxThread);const retryDetails=e.target.closest("[data-retry-details]")?.dataset.retryDetails;if(retryDetails)openDetails(retryDetails);const handlerRole=["BDT_STAFF","CSP_STAFF","CSP_ADMIN","SUPER_ADMIN"].includes(authState.user.role);const take=e.target.closest("[data-take]")?.dataset.take;if(take&&handlerRole)mutateRequest("takeRequest",take,{},`Request ${take} is now processing.`,e.target);const complete=e.target.closest("[data-complete]")?.dataset.complete;if(complete&&handlerRole){const reopen=!!e.target.closest("#details-modal");confirmRequestAction("Complete Request",`Mark ${complete} as completed?`,"Complete Request",button=>mutateRequest("completeRequest",complete,{},`Request ${complete} completed.`,button,{reopenDetails:reopen}))}const unable=e.target.closest("[data-unable]")?.dataset.unable;if(unable&&handlerRole){requestState.reopenAfterMutation=!!e.target.closest("#details-modal");$("#unable-ticket").value=unable;openModal("unable-modal")}const cancel=e.target.closest("[data-cancel]")?.dataset.cancel;if(cancel&&["BDT_STAFF","SUPER_ADMIN"].includes(authState.user.role))confirmRequestAction("Cancel Request",`Cancel pending request ${cancel}?`,"Cancel Request",button=>mutateRequest("cancelRequest",cancel,{},`Request ${cancel} cancelled.`,button));if(e.target.closest("[data-retry-queue]"))loadQueue();if(e.target.closest("[data-retry-bdt-queue]"))loadBdtQueue();if(e.target.closest("[data-close-modal]")||e.target.classList.contains("modal-backdrop"))closeModals();const copy=e.target.closest("[data-copy]")?.dataset.copy;if(copy)copyValue(copy)});
