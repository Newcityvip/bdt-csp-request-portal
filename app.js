...
async function openDetails(ticket){requestState.activeDetailId=ticket;const cached=requestState.detailCache[ticket];$("#details-content").innerHTML=cached?"":detailsSkeleton(ticket);openModal("details-modal");if(cached)renderDetails(cached);try{const data=await authenticatedRead("requestDetails",{requestId:ticket});if(requestState.activeDetailId!==ticket)return;requestState.detailCache[ticket]=data;renderDetails(data);saveSessionCache()}catch(e){if(requestState.activeDetailId!==ticket)return;if(cached){$("#details-content").insertAdjacentHTML("afterbegin",`<div class="details-refresh-warning">Details could not be refreshed. <button class="text-button" data-retry-details="${escapeHtml(ticket)}">Retry</button></div>`)}else $("#details-content").innerHTML=`<div class="details-head details-head-error">${detailsCloseButton()}<h2 id="details-title">Request details</h2></div><div class="details-error"><span>!</span><h3>Unable to load request details</h3><p>${escapeHtml(e.message)}</p><div><button class="secondary-button" data-close-modal>Close</button><button class="primary-button" data-retry-details="${escapeHtml(ticket)}">Retry</button></div></div>`}}
